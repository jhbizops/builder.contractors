...
- `PUBLIC_REGISTRATION_DEFAULT_ROLE` optional non-privileged role override for `/api/auth/register` (defaults to `dual`; privileged values are ignored server-side)
...
- Public registration always assigns the server-side default non-admin role (client-supplied roles are ignored), and admin elevation is restricted to authenticated admins through `PATCH /api/users/:id/promote`.
